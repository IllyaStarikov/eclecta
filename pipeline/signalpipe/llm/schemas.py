...
SYSTEM_CURATE = (
    ...
    "- novelty: what is actually NEW here vs. known background. ONE short "
    "phrase, at most ~12 words, not a full sentence, no trailing period.\n"
...
SYSTEM_JUDGE = (
    ...
    "- channels: which of the listed channels this belongs to.\n"
    "- novelty: what is actually NEW here vs. known background. ONE short "
    "phrase, at most ~12 words, not a full sentence, no trailing period. It "
    "renders as the lead kicker, so keep it tight.\n"
    "- audience: who specifically benefits (one phrase).\n"
...
_DIGEST_CORE = (
    ...
    "- USEFUL: link each item to the ORIGINAL source that broke the story (the "
    "primary document, or the outlet that first reported it) inline as a "
    "markdown link on first mention, and credit a scoop in words too ('first "
    "reported by X'). Whoever broke it gets the credit and the link, never the "
    "aggregator's rewrite. Use ONLY the provided read URLs (never archive.* "
    "links); the provided read URL is already the best FREE read, so every item "
    "leaves the reader a way in even when the primary source is paywalled.\n"
